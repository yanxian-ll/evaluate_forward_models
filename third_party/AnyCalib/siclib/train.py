...
from siclib.utils.experiments import (
    get_best_checkpoint,
    get_last_checkpoint,
    save_experiment,
)
...
def training(rank, conf, output_dir, args):
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_group(
            ...
            timeout=None if args.nccl_timeout is None else timedelta(hours=args.nccl_timeout),
        )
    ...
    while epoch < conf.train.epochs and not stop:
        ...
        is_test_time = epoch % conf.train.test_every_epoch == 0 and (
            epoch > 0 or not args.no_test_0
        )
        # if is_test_time and args.distributed and rank != 0:
        #     torch.distributed.barrier()
        # we first run the eval
        if rank == 0 and is_test_time:
        ...
        if args.distributed:
            ...
            if hasattr(val_loader.sampler, "set_epoch"):
                val_loader.sampler.set_epoch(0)  # some eval pipelines may need this
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nccl_timeout", default=None, type=float, help="NCCL timeout in hours")
