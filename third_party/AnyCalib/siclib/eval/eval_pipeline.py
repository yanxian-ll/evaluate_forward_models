...
class EvalPipeline:
    ...
    def run(self, experiment_dir, model=None, overwrite=False, overwrite_eval=False):
        ...
        if not exists_eval(experiment_dir) or overwrite_eval or overwrite:
            s, f, r = self.run_eval(self.get_dataloader(self.conf.data, 1), pred_file)  # type: ignore
            save_eval(experiment_dir, s, f, r)
        s, r = load_eval(experiment_dir)

        if self.conf.eval.get("delete_cache", False):
            for file in ("results.h5", "predictions.h5"):
                (experiment_dir / file).unlink(missing_ok=True)
            if self.conf.eval.get("delete_also_summaries", False):
                for file in ("summaries.json", "conf.yaml"):
                    (experiment_dir / file).unlink(missing_ok=True)
