"""
A3DScenes Dataset with Instance/Semantic Segmentation using WAI format data.
This dataset extends the base A3DScenesWAI to load instance (semantic) segmentation masks.
"""
...
class A3DScenesWAIInstance(BaseDataset):
    """
    A3DScenes dataset with instance/semantic segmentation support.
    Loads instance segmentation masks (PNG format with category IDs 1-8).
    """
    ...
    def __init__(
        ...
        *args,
        ...
        load_modalities: list = ["image", "depth", "mask", "instance"],
    ...
    def _get_views(self, sampled_idx, num_views_to_sample, resolution):
        ...
        for view_index in view_indices:
            ...
            # Load instance mask from view_data (loaded via load_frame)
            instance_mask = None
            if "instance" in self.load_modalities and "instance" in view_data:
                instance_mask = view_data["instance"].numpy().astype(int)
            ...
            if instance_mask is not None:
                additional_quantities_to_resize.append(instance_mask)
            ...
            # Extract instance mask after resize
            if instance_mask is not None:
                instance_mask = additional_quantities_to_resize[1]
            ...
            # Add instance mask to view dict if available
            if instance_mask is not None:
                view_dict["instance"] = instance_mask
