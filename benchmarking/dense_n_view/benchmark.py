...
from mapanything.utils.metrics import (
    merge_masked_points_list,
    estimate_voxel_size,
    voxel_downsample_np,
    sim3_init_by_nn_umeyama,
    icp_se3_torch,
    chamfer_inlier_torch
)
...
def global_scale_from_pointmaps(
    gt_pts_list, pr_pts_list, masks_list,
...
def get_all_info_for_metric_computation(batch, preds, norm_mode="avg_dis"):
    ...
    ##### build absolute (non-normalized) pose/pts/z-depth to cpu #####
    gt_poses_abs, gt_z_depths_abs, pr_poses_abs, pr_z_depths_abs = [], [], [], []
    gt_pts_abs, pr_pts_abs = [], []

    for i in range(n_views):
        # poses in view0 frame, absolute
        ...
        # pts3d in view0 frame, absolute
        gt_pts_abs.append(no_norm_gt_pts[i].cpu())
        pr_pts_abs.append(no_norm_pr_pts[i].cpu())

        # z depth absolute (use pts3d_cam z, absolute)
        gt_z_depths_abs.append(no_norm_gt_pts3d_cam[i][..., 2:].cpu())
        pr_z_depths_abs.append(no_norm_pr_pts3d_cam[i][..., 2:].cpu())
    ...
    # Estimate global scale s for each batch element using all views
    # We use the *raw absolute* pointmaps in view0 frame: no_norm_gt_pts, no_norm_pr_pts
    # valid_masks is list of (B,H,W) on GPU currently (but we can slice per batch to avoid huge CPU mem)
    ...
    # Build aligned absolute pred info (scaled to gt)
    # Note: scaling affects translation and points; rotation unchanged.
    pr_poses_abs_aligned, pr_pts_abs_aligned, pr_z_depths_abs_aligned = [], [], []
    for i in range(n_views):
        # scale translations
        t_scaled = no_norm_pr_pose_trans[i] * pr_to_gt_scales[:, None]  # (B,3)

        pr_pose_abs = torch.eye(4, device=pr_pose_quats[i].device).unsqueeze(0).repeat(pr_pose_quats[i].shape[0], 1, 1)
        pr_pose_abs[..., :3, :3] = quaternion_to_rotation_matrix(pr_pose_quats[i])
        pr_pose_abs[..., :3, 3] = t_scaled
        pr_poses_abs_aligned.append(pr_pose_abs)

        # scale pointmaps in view0
        s_map = pr_to_gt_scales[:, None, None, None]  # (B,1,1,1)
        pr_pts_abs_aligned.append((no_norm_pr_pts[i] * s_map).cpu())

        # scale local pts3d_cam z
        pr_z_depths_abs_aligned.append((no_norm_pr_pts3d_cam[i][..., 2:] * s_map).cpu())

    pr_info_abs_aligned = {
        "poses": pr_poses_abs_aligned,
        "pts3d": pr_pts_abs_aligned,
        "z_depths": pr_z_depths_abs_aligned,
    }
    ...
    for benchmark_dataset_name, data_loader in data_loaders.items():
        ...
        for dataset_scene in data_loader.dataset.dataset.scenes:
            per_scene_results[dataset_scene] = {
                ...
                "ray_dirs_err_deg": [],
                # Absolute metrics
                "pointmaps_abs_mae": [],       # mean ||pred-gt||   (unit)
                "pointmaps_abs_rmse": [],      # rmse ||pred-gt||   (unit)
                "z_depth_abs_mae": [],         # mean |pred-gt|     (unit)
                "z_depth_abs_rmse": [],        # rmse |pred-gt|     (unit)
                "pose_ate_abs": [],            # ATE in absolute unit
                "merged_pc_abs_chamfer_l1": [],    # absolute chamfer L1 (unit)
                "merged_pc_abs_chamfer_rmse": [],  # absolute chamfer RMSE (unit)
                "merged_pc_abs_inlier_ratio": [],  # ratio (unitless)
                "pr_to_gt_scale": [],
            }
        ...
        for batch in data_loader:
            ...
            gt_info, pr_info, valid_masks, gt_info_abs, pr_info_abs, scale_factors = (
            ...
            for batch_idx in range(batch_size):
                ...
                pointmaps_abs_mae_across_views = []
                pointmaps_abs_rmse_across_views = []
                z_depth_abs_mae_across_views = []
                z_depth_abs_rmse_across_views = []
                ...
                for view_idx in range(n_views):
                    ...
                    gt_pts_abs_v = gt_info_abs["pts3d"][view_idx][batch_idx].numpy()      # (H,W,3)
                    pr_pts_abs_v = pr_info_abs["pts3d"][view_idx][batch_idx].numpy()
                    m_abs = valid_masks[view_idx][batch_idx].numpy().astype(bool)

                    # per-pixel euclidean error
                    e = np.linalg.norm(pr_pts_abs_v - gt_pts_abs_v, axis=-1)  # (H,W)
                    e_valid = e[m_abs]
                    if e_valid.size == 0:
                        point_abs_mae = np.nan
                        point_abs_rmse = np.nan
                    else:
                        point_abs_mae = float(np.mean(e_valid))
                        point_abs_rmse = float(np.sqrt(np.mean(e_valid**2)))
                    
                    # z-depth absolute error
                    gt_z_abs_v = gt_info_abs["z_depths"][view_idx][batch_idx].numpy()[..., 0]  # (H,W)
                    pr_z_abs_v = pr_info_abs["z_depths"][view_idx][batch_idx].numpy()[..., 0]
                    ez = np.abs(pr_z_abs_v - gt_z_abs_v)
                    ez_valid = ez[m_abs]
                    if ez_valid.size == 0:
                        z_abs_mae = np.nan
                        z_abs_rmse = np.nan
                    else:
                        z_abs_mae = float(np.mean(ez_valid))
                        z_abs_rmse = float(np.sqrt(np.mean(ez_valid**2)))
                    
                    # accumulate across views (like your other across_views lists)
                    pointmaps_abs_mae_across_views.append(point_abs_mae)
                    pointmaps_abs_rmse_across_views.append(point_abs_rmse)
                    z_depth_abs_mae_across_views.append(z_abs_mae)
                    z_depth_abs_rmse_across_views.append(z_abs_rmse)
                ...
                # Compute ABS errors (mean across views)
                pointmaps_abs_mae_curr_set = np.nanmean(pointmaps_abs_mae_across_views)
                pointmaps_abs_rmse_curr_set = np.nanmean(pointmaps_abs_rmse_across_views)
                z_depth_abs_mae_curr_set = np.nanmean(z_depth_abs_mae_across_views)
                z_depth_abs_rmse_curr_set = np.nanmean(z_depth_abs_rmse_across_views)
                ...
                # Compute the absolute pose ATE RMSE (using absolute poses in view0 frame)
                ...
                pose_ate_abs_curr_set = evaluate_ate(
                    gt_traj=gt_poses_abs_curr_set, 
                    est_traj=pr_poses_abs_curr_set
                )
                ...
                # Compute merged pointcloud absolute chamfer distance and inlier ratio
                gt_pts_list_abs, pr_pts_list_abs, masks_list = [], [], []
                for view_idx in range(n_views):
                    m = valid_masks[view_idx][batch_idx].numpy().astype(bool)
                    masks_list.append(m)
                    gt_pts_list_abs.append(gt_info_abs["pts3d"][view_idx][batch_idx].numpy())
                    pr_pts_list_abs.append(pr_info_abs["pts3d"][view_idx][batch_idx].numpy())

                gt_merged_abs = merge_masked_points_list(gt_pts_list_abs, masks_list)  # (Ng,3) absolute
                pr_merged_abs = merge_masked_points_list(pr_pts_list_abs, masks_list)  # (Np,3) absolute

                voxel = estimate_voxel_size(gt_merged_abs, max_samples=1500)
                if voxel is None or not np.isfinite(voxel) or voxel <= 0:
                    voxel = 0.01
                voxel = float(voxel * 1.5)

                gt_ds = voxel_downsample_np(gt_merged_abs, voxel)
                pr_ds = voxel_downsample_np(pr_merged_abs, voxel)
                ...
                # --- Sim3 init (optional; allows scale change) ---
                sim3_gate = voxel * 8.0
                s0, R0, t0, pr_sim3 = sim3_init_by_nn_umeyama(
                    pr_t, gt_t, nn_chunk=4096, max_src=20000, max_corr_dist=sim3_gate
                )

                # --- SE3 ICP refine ---
                icp_gate = voxel * 3.0
                R1, t1, pr_refined = icp_se3_torch(
                    pr_sim3, gt_t, iters=20, max_corr_dist=icp_gate,
                    nn_chunk=4096, max_src_corr=30000, trimmed_ratio=0.8
                )

                # Compute the absolute pointcloud absolute chamfer distance and inlier ratio
                inlier_dist = voxel * 2.0
                abs_chamfer_l1, abs_chamfer_rmse, abs_inlier_ratio = chamfer_inlier_torch(
                    pr_refined, gt_t, inlier_dist, nn_chunk=4096, max_eval=20000
                )

                per_scene_results[scene]["merged_pc_abs_chamfer_l1"].append(
                    float(abs_chamfer_l1) if np.isfinite(abs_chamfer_l1) else np.nan
                )
                per_scene_results[scene]["merged_pc_abs_chamfer_rmse"].append(
                    float(abs_chamfer_rmse) if np.isfinite(abs_chamfer_rmse) else np.nan
                )
                per_scene_results[scene]["merged_pc_abs_inlier_ratio"].append(
                    float(abs_inlier_ratio) if np.isfinite(abs_inlier_ratio) else np.nan
                )
                ...
                per_scene_results[scene]["pointmaps_abs_mae"].append(
                    pointmaps_abs_mae_curr_set.item()
                )
                per_scene_results[scene]["pointmaps_abs_rmse"].append(
                    pointmaps_abs_rmse_curr_set.item()
                )
                per_scene_results[scene]["z_depth_abs_mae"].append(
                    z_depth_abs_mae_curr_set.item()
                )
                per_scene_results[scene]["z_depth_abs_rmse"].append(
                    z_depth_abs_rmse_curr_set.item()
                )
                ...
                per_scene_results[scene]["pose_ate_abs"].append(
                    float(pose_ate_abs_curr_set)
                )
                per_scene_results[scene]["pose_auc_5"].append(
                    pose_auc_5_curr_set.item()
                )
                per_scene_results[scene]["pr_to_gt_scale"].append(
                    float(scale_factors["pr_to_gt_scale"][batch_idx].item())
                )
